...
require(DiceKriging)
require(reshape2)
setwd("E:/All important CMG files/Rayligh study/Uncertainty Ra")
data <- read.delim("data.txt")
source('E:/Geostatistics/Chapter4/Code/filled.contour3.R')

#Kriging instead of Polynomial 
Ra.fit <- with(data,
               lm(RaNum_Tmed ~ ReservoirDip + Length + 
...
response.grid <- with(data,RaNum_Tmed)
...
for (i in 1:length(x3.grid)) {
  for (j in 1:length(x4.grid)){
    ...
    mean.response[i,j] = mean(predicted.values.model1[which(design.grid[,3]==x3.grid[i] & 
                                                              design.grid[,4]==x4.grid[j])])
  }
  
}

pdf("Height_Perm_krige.pdf")
...
filled.contour3(x3.grid, x4.grid, mean.response,
                color.palette=topo.colors,xlim=xlim,ylim=ylim,xlab="Height",ylab="Permeability",yaxs='i')
...
for (i in 1:length(x1.grid)) {
  for (j in 1:length(x2.grid)){
    ...
    mean.response[i,j] = mean(predicted.values.model1[which(design.grid[,1]==x1.grid[i] & 
                                                              design.grid[,2]==x2.grid[j])])
  }
  
}

pdf("Dip_Length_krige.pdf")
par(mfrow=c(1,1),pty="s")
adjust=0.001
...
filled.contour3(x1.grid, x2.grid, mean.response,
                color.palette=topo.colors,xlim=xlim,ylim=ylim,xlab="Dip",ylab="Length",yaxs='i')
...
filled.contour3(x1.grid, x3.grid, mean.response,
                color.palette=topo.colors,xlim=xlim,ylim=ylim,xlab="Dip",ylab="Thickness",yaxs='i')
...
for (i in 1:length(x5.grid)) {
  for (j in 1:length(x3.grid)){
    ...
    mean.response[i,j] = mean(predicted.values.model1[which(design.grid[,5]==x5.grid[i] & 
                                                              design.grid[,3]==x3.grid[j])])
  }
  
}

pdf("Porosity_Thickness_krige.pdf")
par(mfrow=c(1,1),pty="s")
adjust=0.001
...
filled.contour3(x5.grid, x3.grid, mean.response,
                color.palette=topo.colors,xlim=xlim,ylim=ylim,xlab="Porosity",ylab="Thickness",yaxs='i')
...
pdf("Porosity_Permeability_krige.pdf")
par(mfrow=c(1,1),pty="s")
adjust=0.001
xlim=c(range(x5.grid)[1]-0.01*range(x5.grid)[1],range(x5.grid)[2]+0.01*range(x5.grid)[2])
ylim=c(range(x4.grid)[1]-.01*range(x4.grid)[1],range(x4.grid)[2]+.12*range(x4.grid)[2])
filled.contour3(x5.grid, x4.grid, mean.response,
                color.palette=topo.colors,xlim=xlim,ylim=ylim,xlab="Porosity",ylab="Permeability",yaxs='i')
